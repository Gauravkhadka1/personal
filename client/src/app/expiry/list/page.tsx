...
// Add filter type with "none" as default
type ServiceFilterType = "none" | "all_microsoft" | "connex" | "idream";
...
const ExpiryList = () => {
  ...
  // Add state for service filter - default to "none" (no filter active)
  const [filterAnchorEl, setFilterAnchorEl] = useState<null | HTMLElement>(
    null,
  );
  const [serviceFilter, setServiceFilter] = useState<ServiceFilterType>("none");
  const [tempServiceFilter, setTempServiceFilter] =
    useState<ServiceFilterType>("none");
  ...
  useEffect(() => {
    if (expiryData?.currentNepaliMonth) {
      setCurrentNepaliMonth(expiryData.currentNepaliMonth);
    }
  }, [expiryData]);

  const { data: clientCounts } = useGetClientCountsQuery();
  const { data: newClientCounts } = useGetNewClientCountsQuery();

  const [currentNepaliMonth, setCurrentNepaliMonth] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsFilterLoading(true); // Set loading state when filter changes
      try {
        await refetch();
        setCurrentPage(1);
      } finally {
        setIsFilterLoading(false); // Clear loading state when done
      }
    };
  ...
  useEffect(() => {
    if (
      filter === "newclient" ||
      filter === "new30days" ||
      Object.values(nepaliMonths)
        .map((m) => m.toLowerCase())
        .includes(filter || "")
    ) {
      setSortConfig({ key: "createdDaysAgo", direction: "asc" });
    }
  }, [filter]);
  ...
  const handleFilterClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    setTempServiceFilter(serviceFilter); // Initialize temp filter with current value
  };
  ...
  const handleFilterClear = () => {
    setTempServiceFilter("none");
    setServiceFilter("none"); // Set to "none" which means no filter active
    handleFilterClose();
    setCurrentPage(1);
  };

  const handleTempFilterChange = (value: ServiceFilterType) => {
    setTempServiceFilter(value);
  };

  const filterOpen = Boolean(filterAnchorEl);
  const filterId = filterOpen ? "service-filter-popover" : undefined;

  // Get active filter label for display
  const getActiveFilterLabel = (): string => {
    switch (serviceFilter) {
      case "all_microsoft":
        return "All Microsoft Services";
      case "connex":
        return "Connex Only";
      case "idream":
        return "iDream Only";
  ...
  // Check if any filter is active (not "none")
  const isFilterActive = (): boolean => {
    return serviceFilter !== "none";
  };
  ...
  // Update the shouldIncludeClient function to include clients without expiring services for new client filters
  const shouldIncludeClient = (client: ClientWithExpiry): boolean => {
    // Handle new30days filter - ALWAYS include these clients regardless of expiring services
    if (filter === "new30days") {
      const createdDays = Number(client.createdDaysAgo);
      return !isNaN(createdDays) && createdDays <= 30;
    }

    // Handle nepali month filters (new clients by month) - ALWAYS include these clients regardless of expiring services
    if (
      filter &&
      Object.values(nepaliMonths)
        .map((m) => m.toLowerCase())
      ...
      // For month filters, we need to check if the client was created in that month
      // This will be handled by the API query, but we should still include all clients returned
      return true;
    }

    // If filter is suspended, always include suspended clients regardless of expiring services
    if (filter === "suspended") {
      return client.status === "suspend";
    }

    // For all other filters (expiry-related filters), exclude suspended clients
    if (client.status === "suspend") return false;

    // If client has no expiring services, include them only if no expiry filter is active
    if (!client.hasExpiringServices) {
      // Check if any expiry filter is active
      const expiryFilters = ["30", "15", "7", "expired"];
      if (filter && expiryFilters.includes(filter)) {
        return false; // Don't show clients without expiring services when expiry filter is active
      }
      return true; // Show clients without expiring services for all other cases
    }

    // Check if client has expiring services that match the filter
    if (filter && client.hasExpiringServices) {
      const expiryFilters = ["30", "15", "7", "expired"];
      if (expiryFilters.includes(filter)) {
  ...
  const shouldIncludeService = (
    ...
    // If this is the "No expiring service" dummy service, don't show it when any filter is active
    if (service.type === "No expiring service") {
      // If any filter is active (URL filter or service filter), don't show "No expiring service"
      const expiryFilters = ["30", "15", "7", "expired"];
      const isExpiryFilterActive = filter && expiryFilters.includes(filter);

      if (isExpiryFilterActive || serviceFilter !== "none") {
        return false;
      }
    }
    ...
    if (serviceFilter !== "none") {
      ...
      // Check if it's a Microsoft service
      const isMicrosoftService =
        serviceTypeLower.includes("microsoft") ||
        serviceTypeLower.includes("connex") ||
        serviceTypeLower.includes("idream");

      // If "all_microsoft" filter is active, only show Microsoft services
      if (serviceFilter === "all_microsoft") {
        return isMicrosoftService;
      }

      // For "connex" filter, show only services that are specifically Connex
      if (serviceFilter === "connex") {
        return serviceTypeLower.includes("connex");
      }

      // For "idream" filter, show only services that are specifically iDream
      if (serviceFilter === "idream") {
        return serviceTypeLower.includes("idream");
      }
    }
  ...
  const getClientExpiryGroups = (): ExpiryGroup[] => {
    ...
    // Sort clients by createdDaysAgo when filter is for new clients
    ...
    if (
      ...
      sortedClients.sort((a, b) => {
        const aDays = Number(a.createdDaysAgo) || 0;
        const bDays = Number(b.createdDaysAgo) || 0;
        return aDays - bDays;
      });
    }
    ...
    sortedClients.forEach((client) => {
      ...
      if (!clientWithExpiry.hasExpiringServices) {
        // Only add "No expiring service" if:
        // 1. No expiry filter is active (30, 15, 7, expired)
        // 2. No service filter is active (connex, idream, all_microsoft)
        const expiryFilters = ["30", "15", "7", "expired"];
        const isExpiryFilterActive = filter && expiryFilters.includes(filter);
        const isServiceFilterActive = serviceFilter !== "none";

        // Don't show "No expiring service" when any filter is active
        if (!isExpiryFilterActive && !isServiceFilterActive) {
          groups.push({
            services: [
              {
  ...
  const sortedGroups = [...expiryGroups].sort((a, b) => {
    ...
    if (sortConfig.key === "createdDaysAgo") {
      aValue = a.client?.createdDaysAgo || 0;
      bValue = b.client?.createdDaysAgo || 0;
      ...
      aValue = new Date(a.expiryDate || 0).getTime();
      bValue = new Date(b.expiryDate || 0).getTime();
  ...
  const getTabCounts = () => {
    ...
    serviceOptions.forEach((option) => {
      if (option.id === "all") {
        counts.all =
          filter === "30"
            ? clientCounts?.expiringIn30Days || 0
            : filter === "15"
              ? clientCounts?.expiringIn15Days || 0
              : filter === "7"
                ? clientCounts?.expiringIn7Days || 0
                : filter === "expired"
                  ? clientCounts?.expired || 0
                  : filter === "suspended"
                    ? clientCounts?.suspended || 0
                    : filter === "new30days"
                      ? pagination.totalCount || 0
                      : clientCounts?.totalClients || 0;
      } else {
    ...
    <Box className="p-4 dark:bg-primary-dark dark:text-gray-300 md:p-6">
      <Box className="mb-4 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
        ...
        <Box className="flex w-full flex-col items-start gap-3 dark:text-gray-300 sm:flex-row sm:items-center md:w-auto">
          ...
          <div className="relative">
            ...
            >
              <SlidersHorizontal className="h-5 w-5 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 cursor-pointer" />
            </button>
            {searchTerm && (
              <button
          ...
          {/* Filter Panel Popup */}
          {isFilterPanelOpen && (
            <div className="absolute right-0 top-36 z-50 mt-2 w-80 rounded-lg border bg-white shadow-lg dark:border-gray-700 dark:bg-secondary">
              <div className="flex items-center justify-between border-b p-4 dark:border-gray-700">
                <h3 className="text-lg font-semibold dark:text-gray-300">Filter by Service</h3>
                <button
              ...
              <div className="p-4">
                <div className="space-y-2">
                  <label className="flex cursor-pointer items-center space-x-3">
                    <input
                      type="radio"
                      checked={serviceFilter === "none"}
                      onChange={() => setServiceFilter("none")}
                      className="h-4 w-4 text-blue-600"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">All Clients</span>
                  </label>
                  
                  <label className="flex cursor-pointer items-center space-x-3 ml-6">
                    <input
                      type="radio"
                      checked={serviceFilter === "all_microsoft"}
                      onChange={() => setServiceFilter("all_microsoft")}
                      className="h-4 w-4 text-blue-600"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">All Microsoft Services</span>
                  </label>
                  
                  <label className="flex cursor-pointer items-center space-x-3 ml-6">
                    <input
                      type="radio"
                      checked={serviceFilter === "connex"}
                      onChange={() => setServiceFilter("connex")}
                      className="h-4 w-4 text-blue-600"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">Connex Only</span>
                  </label>
                  
                  <label className="flex cursor-pointer items-center space-x-3 ml-6">
                    <input
                      type="radio"
                      checked={serviceFilter === "idream"}
                      onChange={() => setServiceFilter("idream")}
                      className="h-4 w-4 text-blue-600"
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">iDream Only</span>
                  </label>
                </div>

                <Link href="/clients/list" className="my-4 block text-sm text-blue-600 hover:underline dark:text-blue-400">
                  Filter Website Client by Category
                </Link>
                ...
                <div className="flex justify-between gap-2">
                  <Button
                    ...
                    className="flex-1 border-gray-300 text-gray-600 hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                  >
          ...
          <FormControl
            size="small"
            className="h-10 min-w-[150px] border-gray-300 dark:border-gray-600"
          >
            ...
            >
              ...
              <MenuItem value={20} className="dark:hover:bg-gray-800">
                20
              </MenuItem>
              ...
              <MenuItem value={500} className="dark:hover:bg-gray-800">
                500
              </MenuItem>
            </Select>
          </FormControl>
      ...
      {(filter === "newclient" ||
        ...
        Object.values(nepaliMonths)
          .map((m) => m.toLowerCase())
          .includes(filter || "")) && (
        <Box className="mb-4 border-b border-gray-200 dark:border-gray-700">
          <Tabs
            value={currentNepaliMonth || filter}
            onChange={(e, newValue) => {
              router.push(`/expiry/list?filter=${newValue}`);
            }}
            className="min-h-0"
            sx={{
              minHeight: "40px",
          ...
          >
            {newClientCounts?.monthlyCounts.map((monthData) => (
              <Tab
                key={monthData.month}
                value={monthData.monthName.toLowerCase()}
                label={
                  <div className="relative flex items-center justify-center gap-2">
                    <span className="mr-2 whitespace-nowrap dark:text-gray-300">
                      {monthData.monthName}
                    </span>
                    {monthData.count > 0 && (
                      <span className="absolute -right-6 -top-2 min-w-[26px] rounded-full bg-green-100 px-2 py-1 text-center text-xs font-semibold text-green-800 dark:bg-green-50 dark:text-green-600">
                        {monthData.count}
                      </span>
                    )}
                  </div>
      ...
      <TableContainer
        ...
        elevation={0}
        sx={{ maxHeight: "calc(100vh - 190px)" }} // Add fixed height for scroll
      >
        <Table
          className="min-w-[1000px]"
          size="small"
          stickyHeader // Add this prop to make the header sticky
        >
          <TableHead>
            <TableRow className="bg-gray-200 dark:bg-secondary-dark">
              ...
              <TableCell className="dark:border-gray-700">
                <TableSortLabel
                  ...
                  IconComponent={() => getSortIcon("daysLeft")}
                  className="font-semibold text-gray-700 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
                >
                  <p className="text-lg font-semibold text-gray-600 dark:text-gray-300">
                    Days Left
            ...
            {isLoading || isFilterLoading ? (
              ...
              <TableRow>
                <TableCell
                  colSpan={8}
                  align="center"
                  className="p-6 text-lg font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400"
                >
                  {searchTerm ? (
                    <>
                      <p className="text-lg">
                  ...
                  {isFilterActive() && (
                    <p className="mt-2 text-sm text-gray-400">
                      Active filter: {getActiveFilterLabel()}
                    </p>
                  )}
                </TableCell>
              </TableRow>
                ...
                return (
                  ...
                  >
                    <TableCell className="cursor-pointer pl-3 dark:border-gray-700 dark:text-gray-300">
                      <div className="flex w-[260px] items-center justify-start gap-2">
                        <a
                        ...
                        {(filter === "newclient" ||
                          ...
                          Object.values(nepaliMonths)
                            .map((m) => m.toLowerCase())
                            .includes(filter || "")) &&
                          client.createdDaysAgo !== undefined && (
                            <span className="flex w-[33%] rounded-lg bg-green-100 px-2 py-1 text-sm font-bold text-green-700 dark:bg-green-900/30 dark:text-green-400">
                              {client.createdDaysAgo}d ago
                            </span>
                          )}
                      </div>
      ...
      {pagination.totalPages > 1 && (
        <Box className="mt-4 flex flex-col items-center justify-end gap-4 px-1 sm:flex-row">
          {/* <Typography
            variant="body2"
      ...
      {/* Suspend/Activate Client Confirmation Dialog */}
      ...
      {/* Renew Service Confirmation Dialog  */}
