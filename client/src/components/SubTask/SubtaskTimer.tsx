...
import TimerConfirmationDialog from "./TimerConfirmationDialog";
...
const SubtaskTimer = ({
  subtaskId,
  initialIsRunning,
  initialTimerStartTime,
  initialTimeSpent,
}: SubtaskTimerProps) => {
  const { user } = useAuth();
  const [startSubtaskTimer] = useStartSubtaskTimerMutation();
  const [pauseSubtaskTimer] = usePauseSubtaskTimerMutation();
  const [stopSubtaskTimer] = useStopSubtaskTimerMutation();
  ...
  const { data: timerStatus, refetch: refetchTimer } =
    useGetSubtaskTimerStatusQuery(subtaskId);
  ...
  const handleStartTimer = async () => {
    try {
      const result = await startSubtaskTimer({
        subtaskId,
      ...
      console.error("Failed to start subtask timer:", error);
      if (error.status === 409) {
        setConfirmationDialog({
          isOpen: true,
        ...
        toast.error(error.data?.message || "Failed to start subtask timer");
      }
    }
  };
  ...
  const handleConfirmSwitch = async () => {
    try {
      await stopSubtaskTimer({
        subtaskId: confirmationDialog.runningTaskId,
        userId: Number(user?.userId),
      }).unwrap();

      await startSubtaskTimer({
        subtaskId,
        userId: Number(user?.userId),
      }).unwrap();
  ...
  const handlePauseTimer = async () => {
    try {
      await pauseSubtaskTimer({
        subtaskId,
        userId: Number(user?.userId),
      }).unwrap();
      setLocalStartTime(null); // Clear local start time when pausing
      refetchTimer();
      toast.success("Subtask timer paused");
    } catch (error: any) {
      console.error("Failed to pause subtask timer:", error);
      toast.error(error.data?.message || "Failed to pause subtask timer");
    }
  };
  ...
  const handleStopTimer = async () => {
    try {
      ...
      refetchTimer();
      toast.success("Subtask timer stopped");
    } catch (error: any) {
      console.error("Failed to stop subtask timer:", error);
      toast.error(error.data?.message || "Failed to stop subtask timer");
    }
  };
        ...
        <div className={`absolute inset-0 flex items-center justify-center gap-1 transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
          {!localTimer.isTimerRunning ? (
            <button
              ...
              className="p-1 text-green-600 hover:bg-green-100 rounded-md transition-colors dark:hover:bg-green-900/30"
              title="Start Timer"
            >
              <Play size={14} />
            </button>
            ...
            <button
              ...
              className="p-1 text-yellow-600 hover:bg-yellow-100 rounded-md transition-colors dark:hover:bg-yellow-900/30"
              title="Pause Timer"
            >
              <Pause size={14} />
            </button>
          ...
          <button
            ...
            className="p-1 text-red-600 hover:bg-red-100 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed dark:hover:bg-red-900/30"
            title="Stop Timer"
          >
            <Square size={14} />
          </button>
